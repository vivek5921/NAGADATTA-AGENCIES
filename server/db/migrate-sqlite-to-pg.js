...
async function migrateSqliteToPg() {
  ...
  try {
    ...
    // 1. Check & Migrate Users
    const pgUsers = await getAll(`SELECT * FROM users`);
    if (pgUsers.length === 0) {
      ...
      console.log(`[MIGRATION-IMPORT] Imported ${sqliteUsers.length} user(s) into PostgreSQL.`);
    }

    // 2. Check & Migrate Shop Settings
    const pgSettings = await getAll(`SELECT * FROM shop_settings`);
    if (pgSettings.length === 0) {
      ...
      console.log(`[MIGRATION-IMPORT] Imported ${sqliteSettings.length} shop setting(s) into PostgreSQL.`);
    }

    // 3. Check & Migrate Categories
    const pgCats = await getAll(`SELECT * FROM categories`);
    if (pgCats.length === 0) {
      ...
      console.log(`[MIGRATION-IMPORT] Imported ${sqliteCats.length} category/categories into PostgreSQL.`);
    }

    // 4. Check & Migrate Products
    const pgProds = await getAll(`SELECT * FROM products`);
    if (pgProds.length === 0) {
      ...
      console.log(`[MIGRATION-IMPORT] Imported ${sqliteProds.length} product(s) into PostgreSQL.`);
    }

    // 5. Check & Migrate Spare Parts
    const pgParts = await getAll(`SELECT * FROM spare_parts`);
    if (pgParts.length === 0) {
      ...
      console.log(`[MIGRATION-IMPORT] Imported ${sqliteParts.length} spare part(s) into PostgreSQL.`);
    }
